...
export default function RoleDisplay({ role }: RoleProps) {
  ...
  return (
    <div
      className={roleStyle.roleContainer}
    ...
    >
      ...
      <div className={'role-name'}>
        <h5 property={'roleName'}>{role.roleName}</h5>
      </div>
      <div className={roleStyle.roleOrganization}>
        <h5 property={'name'}>{role.organization}</h5>
        <div
          className={roleStyle.roleOrganizationLocation}
          property={'address'}
        >
      ...
      <div className={roleStyle.roleDuration}>
        <span className={roleStyle.started} property={'startDate'}>
          {role.startYear}
        </span>
        {!isNotCurrent ? (
          <span className={roleStyle.current}>Current</span>
        ) : (
          <span className={roleStyle.ended} property={'endDate'}>
            {role.endYear}
          </span>
        )}
      </div>
      {hasDescription && (
        <div
          ...
          className={roleStyle.roleDescription}
          dangerouslySetInnerHTML={ComponentHelper.getHTML(role)}
        ></div>
      )}
    </div>
  )
